...
import {
  MapPin, AlertTriangle, ChevronLeft, ChevronRight, ArrowLeft, Frown,
  Tag, BadgeCheck,
} from 'lucide-react'
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      const res = await api.get<{
        ok: boolean
        data: { ads: AdItem[]; total: number; pages: number; city?: { name: string } }
      }>(`/cities/${slug}/ads?page=${page}&limit=12`)
      setAds(res.data.data.ads)
      setTotal(res.data.data.total)
      setPages(res.data.data.pages)
      if (res.data.data.city?.name) setCityName(res.data.data.city.name)
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } }).response?.data?.error
      setError(msg ?? 'Failed to load city ads')
    } finally {
      setLoading(false)
    }
  }, [slug, page])
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-heading text-xl font-bold text-foreground">
                {cityName || slug?.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())}
              </h1>
        ...
        {error ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="font-heading text-sm font-semibold text-foreground">No listings in this city</p>
          </div>
        ) : (
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
